...
static struct futex_hash_bucket *futex_hash_private_get_hb(union futex_key *key)
{
...
#if 0
	If we use multiple hash_slots lists + locks to spread the lock
	contention a little.
	...
	list_for_each_entry(fhs, &fh->hash_slots, list) {
		if (fhs->uaddr == key->private.address) {
			fhs->user++;
	...
	list_add(&fhs->list, &fh->hash_slots);
	raw_spin_unlock(&fh->hash_lock);
	return &fhs->hb;
}
...
void futex_mm_hash_exit(struct mm_struct *mm)
{
	...
	BUG_ON(!list_empty(&fh->hash_slots));
}
#endif
